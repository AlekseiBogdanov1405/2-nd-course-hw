...
// let a = Number (prompt ("Цифру?"));
// let b = Number (prompt ('Еще какую-нибудь цифру?'));
// alert (a+b);
...
// alert (`Через год вам будет ${age+1}`);
...
// // **Не забудьте про использование ****инкремента / декремента* НЕ ПОЛУЧИЛОСЬ С НИМИ
...
// let salary = Number(prompt ('Подскажите разерм вашей заработной платы?'));
// alert (`Ваша премия: ${salary*1.2}`)
// salary*= 0.87;
// alert (`Подоходный налог 13%: ${salary}`);
// alert (`Бюджет на день: ${salary/30}`);
...
class Car {
    ...
    getCar = () => {
        console.log(`Машина ${this.model} едет со скоростью ${this.speed}`);
    }
    
}

const bmw = new Car('Bmw m5', '250');
bmw.getCar()